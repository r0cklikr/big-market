...
public class ActivityRepository implements IActivityRepository {
    ...
    public void cacheActivitySkuStockCount(String cacheKey, Integer stockCount) {
        if(!redissonClient.getBucket(cacheKey).isExists()){
            log.info("不存在sku库存,值:{}",stockCount);
            redissonClient.getAtomicLong(cacheKey).set(stockCount);
            log.info("修改后的值:{}",redissonClient.getAtomicLong(cacheKey).get());
        }
    }
    ...
    public boolean subtractionActivitySkuStock(Long sku, String cacheKey, Date endDateTime) {
        ...
        if (surplus == 0) {
            ...
            //publish(activitySkuStockZeroMessageEvent.topic(), activitySkuStockZeroMessageEvent.buildEventMessage(sku));
            log.info("发送消息:{}",sku);
            rabbitTemplate.convertAndSend("sku.stock.topic","reset.stock",sku);
            return false;
        } else if (surplus < 0) {
            // 库存小于0，恢复为0个
        ...
        Boolean lock = redissonClient.getBucket(lockKey).setIfAbsent("lock", Duration.ofMillis(expireMillis));
        if (!lock) {
